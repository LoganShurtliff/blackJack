...
const shuffleDeck = (deck) => {
  let currentPosition = deck.length,
    randomPosition;
...
const Deal = () => {
  ...
  playerScoreP.classList.remove("hidden");
  dealerScoreP.classList.remove("hidden");
...
const updateUI = () => {
  ...
  dealerHand.forEach((card) => {
    dealerHtml += `<img src="imgs/${card}.svg">`;
  });
  dealerDiv.innerHTML += dealerHtml + "</div>";

  let playerHtml = "<div>";
  playerHand.forEach((card) => {
    playerHtml += `<img src="imgs/${card}.svg">`;
  });
  playerDiv.innerHTML += playerHtml + "</div>";
...
const bust = () => {
  ...
  playerScoreP.classList.add("hidden");
...
const Hold = () => {
  ...
  if (dealerScore.length == 0) {
    //If the dealer went over 21 and has no valid scores
    playerWin();
  } else if (dealerScore[dealerScore.length - 1] > playerScore[playerScore.length - 1]) {
    dealerWin();
  } else {
